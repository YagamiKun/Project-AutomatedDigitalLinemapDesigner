...
namespace RenderHeads.Media.AVProMovieCapture.Editor
{
	...
	public class CaptureEditorWindow : EditorWindow
	{
		...
		private	SerializedProperty _propVideoHintsEnableConstantQuality;
...
#elif UNITY_EDITOR_OSX
				_propVideoHintsEnableConstantQuality = _so.AssertFindProperty("_encoderHints.videoHints.enableConstantQuality");
		...
		private void LoadSettings()
		{
			...
			_encoderHints.videoHints.enableConstantQuality = EditorPrefs.GetBool(SettingsPrefix + "EncoderHints.VideoHints.EnableConstantQuality", _encoderHints.videoHints.enableConstantQuality);
		...
		private void SaveSettings()
		{
			...
			EditorPrefs.SetBool(SettingsPrefix + "EncoderHints.VideoHints.EnableConstantQuality", _encoderHints.videoHints.enableConstantQuality);
		...
		private void Configure(CaptureBase capture)
		{
			...
			if (_outputFolderIndex == (int)CaptureBase.OutputPath.RelativeToPersistentData)
			{
		...
		{
			//Debug.Log("Creating Hidden Object");
			...
			{
                // might have to the find here to to ensure that it uses the correct timeline controller is used
                if (_timelineController == null)
				{
			...
			switch (_sourceType)
			{
				case SourceType.Screen:
					// need to check to see if their any of these in the scene and then just set to that if there is
#if UNITY_HAS_FINDFIRSTOBJECTBYTYPE
					var preExistingCapScreen = FindFirstObjectByType<CaptureFromScreen>();
#else
					var preExistingCapScreen = (CaptureFromScreen)GameObject.FindObjectOfType(typeof(CaptureFromScreen));
#endif
					if (preExistingCapScreen)
						preExistingCapScreen.enabled = false;
				...
				case SourceType.Camera:
#if UNITY_HAS_FINDFIRSTOBJECTBYTYPE
                    var preExistingCapCamera = FindFirstObjectByType<CaptureFromCamera>();
#else
					var preExistingCapCamera = (CaptureFromCamera)GameObject.FindObjectOfType(typeof(CaptureFromCamera));
#endif
                    if (preExistingCapCamera)
                        preExistingCapCamera.enabled = false;
					...
					// might have to the find here to to ensure that it uses the correct selector
...
#if UNITY_HAS_FINDFIRSTOBJECTBYTYPE
                    var preExistingCapCamera360 = FindFirstObjectByType<CaptureFromCamera360>();
#else
					var preExistingCapCamera360 = (CaptureFromCamera360)GameObject.FindObjectOfType(typeof(CaptureFromCamera360));
#endif
                    if (preExistingCapCamera360)
                        preExistingCapCamera360.enabled = false;
...
#if UNITY_HAS_FINDFIRSTOBJECTBYTYPE
                    var preExistingCapCamera360DS = FindFirstObjectByType<CaptureFromCamera360ODS>();
#else
					var preExistingCapCamera360DS = (CaptureFromCamera360ODS)GameObject.FindObjectOfType(typeof(CaptureFromCamera360ODS));
#endif
                    if (preExistingCapCamera360DS)
                        preExistingCapCamera360DS.enabled = false;
...
#endif
            // need to check if their is a GUI component in the scene then if their is set disable it as this component, handles the UI
			// so no need for an in scene one
#if UNITY_HAS_FINDFIRSTOBJECTBYTYPE
...
#endif
            if (testingGUI != null)
				testingGUI.enabled = false;

			// if their is a capture component of the same type should probably use that rather than creating a new one
        }
		...
		void OnInspectorUpdate()
		{
			if (_capture != null)
			{
				if (Application.isPlaying)
				{
					...
					// RJT NOTE: Added further '!EditorApplication.isPaused' check for: https://github.com/RenderHeads/UnityPlugin-AVProMovieCapture/issues/392
					if (!EditorApplication.isPaused && (!_capture.IsPrepared() ||
						(_capture.StopMode != StopMode.None && _capture.CaptureStats.NumEncodedFrames > 0 && !_capture.IsCapturing() && !_capture.IsStartCaptureQueued()))
					)
					{
						StopCapture();
					}
		...
		public void DrawMoreCapturingGUI()
		{
			...
			if (IsImageSequence())
			{
				...
				EditorGUILayout.LabelField("Threads", _capture.CaptureStats.NumBufferedFrames.ToString() + " / " + _capture.CaptureStats.TotalBuffersAvailable.ToString());
				// max is 64 (8x8)
				// Key:
				//	o - active not in use
				//	x - active in use
				//	- - not active
				//UInt64 active = _capture.CaptureStats.ActiveWorkers;
				//int max = _capture.CaptureStats.MaxActiveWorkers;
				//int total = 0;
				//Debug.Log(_capture.CaptureStats.ActiveWorkers);
				//for (int i = 0; i < 8; i++) // row
				//{
				//	EditorGUILayout.BeginHorizontal();
				//	for (int j = 0; j < 8; j++) // column
				//	{
				//		UInt64 currentBit = (UInt64)(1 << total);
				//		if (total > max)
				//			GUILayout.Label("-");
				//		else if ((active & currentBit) == currentBit)
				//			GUILayout.Label("x");
				//		else
				//			GUILayout.Label("o");
				//		total++;
		...
		public static void DrawConfigGUI_About()
		{
            ...
            var centreStyle = new GUIStyle(GUI.skin.label);
            centreStyle.alignment = TextAnchor.MiddleCenter;
            var centreBoldStyle = new GUIStyle(GUI.skin.label);
            centreBoldStyle.alignment = TextAnchor.MiddleCenter;
            centreBoldStyle.fontStyle = FontStyle.Bold;
            var centreTitleStyle = new GUIStyle(GUI.skin.label);
            centreTitleStyle.alignment = TextAnchor.MiddleCenter;
            centreTitleStyle.fontStyle = FontStyle.Bold;
            centreTitleStyle.fontSize = 20;

            EditorGUILayout.BeginVertical();
			{
				DrawLogoInfo(centreStyle, centreBoldStyle, centreTitleStyle);
				EditorGUILayout.Space(10);
				DrawLinks(centreStyle, centreBoldStyle, centreTitleStyle);
                EditorGUILayout.Space(20);
                DrawCredits(centreStyle, centreBoldStyle, centreTitleStyle);
                EditorGUILayout.Space(10);
                DrawBugReort(centreStyle, centreBoldStyle, centreTitleStyle);
			} 
            EditorGUILayout.EndVertical();
		}
		private static void DrawLogoInfo(GUIStyle centreStyle, GUIStyle centreBoldStyle, GUIStyle centreTitleStyle)
		{
            ...
            EditorGUILayout.BeginVertical(GUILayout.MaxWidth(EditorGUIUtility.currentViewWidth));
			{
                ...
                if (_icon == null)
					_icon = Resources.Load<Texture2D>("AVProMovieCaptureIcon");
				if (_icon != null)
					GUILayout.Label(new GUIContent(_icon), centreStyle);

				GUI.color = Color.yellow;
				EditorGUILayout.LabelField("AVPro Movie Capture by RenderHeads Ltd", centreBoldStyle);
				GUI.color = Color.white;

				GUI.color = Color.yellow;
                EditorGUILayout.LabelField("version " + version + " (scripts v" + NativePlugin.ScriptVersion + ")", centreStyle);
				GUI.color = Color.white;
			}
			EditorGUILayout.EndVertical();
		}
		private static void DrawCredits(GUIStyle centreStyle, GUIStyle centreBoldStyle, GUIStyle centreTitleStyle)
		{
            EditorGUILayout.LabelField("Credits", centreTitleStyle);
            EditorGUILayout.BeginVertical(GUILayout.MaxWidth(EditorGUIUtility.currentViewWidth));
			{
                EditorGUILayout.LabelField("Programming", centreBoldStyle);
				EditorGUILayout.LabelField("Andrew Griffiths", centreStyle);
				EditorGUILayout.LabelField("Morris Butler", centreStyle);
				EditorGUILayout.LabelField("Richard Turnbull", centreStyle);
				EditorGUILayout.LabelField("Sunrise Wang", centreStyle);
				EditorGUILayout.LabelField("Reuben Miller", centreStyle);

				EditorGUILayout.LabelField("Graphics", centreBoldStyle);
				EditorGUILayout.LabelField("Jeff Rusch", centreStyle);
				EditorGUILayout.LabelField("Luke Godward", centreStyle);
			}
            GUILayout.FlexibleSpace();
            EditorGUILayout.EndVertical();
        }
		private static void DrawLinks(GUIStyle centreStyle, GUIStyle centreBoldStyle, GUIStyle centreTitleStyle)
		{
			float width = EditorGUIUtility.currentViewWidth;
			Vector2 buttonSize = new(100, 50);

			EditorGUILayout.BeginVertical(GUILayout.MaxWidth(EditorGUIUtility.currentViewWidth));
            {
				EditorGUILayout.Space(10f);
                EditorGUILayout.LabelField("AVPro Movie Capture Links", centreTitleStyle, GUILayout.MaxHeight(25));
				GUILayout.Space(10f);
				GUILayout.BeginHorizontal(GUILayout.MaxWidth(width)); 

                GUILayout.FlexibleSpace();
                EditorUtils.DrawURLButton(buttonSize, LinkUserManual,       /*icon: "d_Help@2x",*/ text: "User Manual");
                EditorUtils.DrawURLButton(buttonSize, LinkSupport,			/*icon: "console.infoicon@2x",*/ text: "GitHub Issues");
                EditorUtils.DrawURLButton(buttonSize, LinkAssetStorePage,	/*icon: "d_Asset Store@2x",*/ text: "Asset Store");
                GUILayout.FlexibleSpace();									
                GUILayout.EndHorizontal();									
                GUILayout.BeginHorizontal(GUILayout.MaxWidth(width));		
                GUILayout.FlexibleSpace();									
                EditorUtils.DrawURLButton(buttonSize, LinkForumPage,		/*icon: "Git@2x",*/ text: "Forum Thread");
				EditorUtils.DrawURLButton(buttonSize, LinkPluginWebsite,    /*icon: "Profiler.GlobalIllumination@2x",*/ text: "Official Website");
                GUILayout.FlexibleSpace();
                GUILayout.EndHorizontal();
            }
            EditorGUILayout.EndVertical();
		}
		private static void DrawBugReort(GUIStyle centreStyle, GUIStyle centreBoldStyle, GUIStyle centreTitleStyle)
        {
			EditorGUILayout.BeginVertical(GUILayout.MaxWidth(EditorGUIUtility.currentViewWidth));
            EditorGUILayout.LabelField("Bug Reporting Notes", centreTitleStyle, GUILayout.MaxHeight(25));
			EditorGUILayout.Space(10f);
		...
		private void DrawConfigGUI_Encoding()
		{
				...
				_propOutputTarget.enumValueIndex == (int)OutputTarget.ImageSequence)
			{
...
#elif UNITY_EDITOR_OSX
					EditorGUILayout.PropertyField(_propVideoHintsEnableConstantQuality);
				...
				else if (_propOutputTarget.enumValueIndex == (int)OutputTarget.ImageSequence)
				{
					...
					if (_propImageSequenceFormat.enumValueIndex == (int)ImageSequenceFormat.PNG)
					{
						// windows no uses fpng for image processing, which does not support PNG
#if UNITY_EDITOR_OSX
						EditorGUILayout.BeginHorizontal();
						EditorGUILayout.LabelField("Quality");
						_encoderHints.imageHints.quality = EditorGUILayout.IntSlider((int)_encoderHints.imageHints.quality, 0, 100);
						EditorGUILayout.EndHorizontal();
#else
						EditorGUILayout.BeginHorizontal();
                        EditorGUILayout.LabelField("Compressed");
                        bool val = _encoderHints.imageHints.quality >= 1 ? true : false;
                        val = EditorGUILayout.ToggleLeft("", val);
                        _encoderHints.imageHints.quality = val ? 1 : 0;
                        EditorGUILayout.EndHorizontal();
#endif
                    }
					else if (_propImageSequenceFormat.enumValueIndex == (int)ImageSequenceFormat.JPEG)
					{
						if (_encoderHints.imageHints.quality > 1)
							_encoderHints.imageHints.quality = 1;
                        EditorGUILayout.PropertyField(_propImageHintsQuality);
                    }
#endif
						if (_propImageSequenceFormat.enumValueIndex != (int)ImageSequenceFormat.JPEG)
					{
		...
		private bool IsImageSequence()
		{
			return _outputTarget == OutputTarget.ImageSequence;
		}
