...
namespace RenderHeads.Media.AVProMovieCapture.Editor
{
	...
	public static class EditorUtils
	{
        ...
        public static void DrawSection(string name, ref bool isExpanded, System.Action action, ref AnimBool anim)
        {
            Color boxbgColor = new Color(0.8f, 0.8f, 0.8f, 0.1f);
            if (EditorGUIUtility.isProSkin)
            {
            ...
            DrawSectionColored(name, ref isExpanded, action, boxbgColor, Color.white, Color.white, ref anim);
        }
        ...
        public static void DrawSectionColored(string name, ref bool isExpanded, System.Action action, Color boxbgcolor, Color bgcolor, Color color, ref AnimBool anim)
        {
            GUI.color = Color.white;
            GUI.backgroundColor = Color.clear;
            //GUI.backgroundColor = bgcolor;
		...
		public static void DrawURLButton(Vector2 size, string url, string icon = "", string text = "", GUIStyle labelStyle = null)
		{
			if (labelStyle == null)
			...
			if (!string.IsNullOrEmpty(icon))
			{
                ...
                if (GUILayout.Button(new GUIContent(text, image), GUILayout.MaxWidth(size.x), GUILayout.MaxHeight(size.y)))
					Application.OpenURL(url);
			}
			else
			{
                if (GUILayout.Button(text, GUILayout.MaxWidth(size.x), GUILayout.MaxHeight(size.y)))
                    Application.OpenURL(url);
            }
