...
namespace RenderHeads.Media.AVProMovieCapture
{
	...
	public class CaptureFromCamera360ODS : CaptureBase
	{
		...
		public class Settings
		{
			...
			[SerializeField]
			public bool SupportWatermark = false;
        }
        ...
        // WATERMARK
        private Material _watermarkBlitMaterial;
        public  CustomWatermark watermark;
		...
		public override void Start()
		{
			...
			{
				Shader mergeShader;
				mergeShader = Shader.Find("Hidden/AVProMovieCapture/ODSMerge");
			...
			if (!_watermarkBlitMaterial && _settings.SupportWatermark)
			{
				Shader watermarkBlitShader = Shader.Find("Hidden/AVProMovieCapture/ODSWatermark");
				_watermarkBlitMaterial = new Material(watermarkBlitShader);
            }
		...
		public (Camera, Camera, Camera, Camera) GetEyeCameras()
		{
			return (_leftCameraTop, _leftCameraBot, _rightCameraTop, _rightCameraBot);
		}
		...
		private void RenderFrame()
		{
			...
			//if (watermark != null)
			//{
			//	watermark.UpdatePosition = true;
			//	var off = watermark.GetOffset();
			//	_finalMaterial.SetTexture("_WatermarkTex", watermark.Texture);
			//	_finalMaterial.SetVector("_WatermarkRect", new Vector4(off.Item2.x, off.Item2.y, off.Item1.x, off.Item1.y));
			//}
			...
			// Do watermark things if the watermark is enabled (and has a material)
            if (watermark != null && _settings.SupportWatermark)
            {
                watermark.UpdatePosition = true;
                var off = watermark.GetOffset();
                _watermarkBlitMaterial.SetTexture("_MainTex", _final);
                _watermarkBlitMaterial.SetTexture("_Watermark", watermark.Texture);
                _watermarkBlitMaterial.SetFloat("_Face", watermark.RenderToFace);
                _watermarkBlitMaterial.SetVector("_Position", new Vector4(off.Item2.x, off.Item2.y, 0, 0));
                _watermarkBlitMaterial.SetVector("_Size", new Vector4(off.Item1.x, off.Item1.y, 0, 0));
                RenderTexture temp = RenderTexture.GetTemporary(_final.width, _final.height, 0, _final.format);
                Graphics.Blit(_final, temp, _watermarkBlitMaterial);
                Graphics.Blit(temp, _final);
		...
		public override bool PrepareCapture()
		{
			...
			if ( !NativePlugin.IsBasicEdition() )
			{
				...
				// Cameras might need transparency
				if( _Transparency != Transparency.None )
				{
					_leftCameraTop.backgroundColor = new Color(0f, 0f, 0f, 0f);
					_leftCameraBot.backgroundColor = new Color(0f, 0f, 0f, 0f);
					_rightCameraTop.backgroundColor = new Color(0f, 0f, 0f, 0f);
					_rightCameraBot.backgroundColor = new Color(0f, 0f, 0f, 0f);
				}
