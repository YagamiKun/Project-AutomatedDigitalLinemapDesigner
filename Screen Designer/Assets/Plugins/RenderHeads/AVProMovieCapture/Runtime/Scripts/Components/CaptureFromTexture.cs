...
namespace RenderHeads.Media.AVProMovieCapture
{
	...
	public class CaptureFromTexture : CaptureBase
	{
		...
		// RBN:
		//  - Added to allow for the blitting of the custom watermark
			...
			if (ShouldCaptureFrame())
			{
				...
				if (hasSourceTextureChanged)
				{
					if ((_manualUpdate /*&& NativePlugin.IsNewFrameDue(_handle)*/) || CanOutputFrame())
					{
						...
						// Add watermark to the texture if the object is active, meaning it has been set
						if (customWatermark)
							sourceTexture = customWatermark.AddWatermarkToTexture((RenderTexture)sourceTexture);
