...
namespace RenderHeads.Media.AVProMovieCapture
{
	...
	public class CaptureGUI : MonoBehaviour
	{
		...
		private void MyWindow(int id)
		{
			...
			if (_movieCapture != null)
			{
				...
				else if (_movieCapture.OutputTarget == OutputTarget.ImageSequence)
				{
					...
					// Transparency supported on all formats even if the codec option may be unavailable
					GUILayout.BeginHorizontal();
					GUILayout.Label("Transparency:", GUILayout.ExpandWidth(false));
					_selectedTransparency = GUILayout.SelectionGrid(_selectedTransparency, NativePlugin.TransparencyNames, 4);
					_movieCapture.GetEncoderHints().imageHints.transparency = (Transparency)_selectedTransparency;
					GUILayout.EndHorizontal();

					switch (_movieCapture.NativeImageSequenceFormat)
					{
						case ImageSequenceFormat.PNG:
						#if UNITY_STANDALONE_WIN || UNITY_EDITOR_WIN
							bool val = _movieCapture.GetEncoderHints().imageHints.quality >= 1;
							val = GUILayout.Toggle(val, "Compressed");
							_movieCapture.GetEncoderHints().imageHints.quality = val ? 1 : 0;
						#endif
							break;
						...
						case ImageSequenceFormat.JPEG:
							...
							GUILayout.BeginHorizontal();
							var quality = Mathf.Clamp01(_movieCapture.GetEncoderHints().imageHints.quality);
							quality = GUILayout.HorizontalSlider(quality, 0.1f, 1f);
							GUILayout.Label(quality.ToString());
							_movieCapture.GetEncoderHints().imageHints.quality = quality;
							GUILayout.EndHorizontal();
							break;
					}
				...
				else
				{
					if (!string.IsNullOrEmpty(CaptureBase.LastFileSaved))
					{
						...
						GUI.enabled = CaptureBase.LastFileSaved.Length > 0;
						if (GUILayout.Button("View Last Capture"))
						{
#if !UNITY_EDITOR && (UNITY_ANDROID || UNITY_IOS)
#if UNITY_IOS
							string lastFileSaved = CaptureBase.LastFileSaved;
							if (!lastFileSaved.StartsWith("file://"))
							{
								// Handheld.PlayFullScreenMovie requires a URL so need to make sure lastFileSaved is
								// correctly percent encoded.
								lastFileSaved = "file://" + HttpUtility.UrlPathEncode(lastFileSaved);;
							}
#else
							string lastFileSaved = CaptureBase.LastFileSaved;
#endif
							string extension = Path.GetExtension(lastFileSaved).ToLower();
							if (extension == ".mp4" || extension == ".mov")
							{
								Debug.LogFormat("Attempting to open '{0}'", lastFileSaved);
								Handheld.PlayFullScreenMovie(lastFileSaved, Color.black, FullScreenMovieControlMode.CancelOnInput);
							}
							else
#endif
							{
		...
		private void GUI_RecordingStatus()
		{
			...
			// what to show the amount of encoded frames if within performing and ImageSequence Capture
			if (_movieCapture.OutputTarget == OutputTarget.ImageSequence)
            {
				DrawGuiField("Encoded Frames", _movieCapture.CaptureStats.NumEncodedFrames.ToString());
			}
			else
			{
		...
		private void DrawPauseResumeButtons()
		{
			...
			GUI.backgroundColor = Color.cyan; // this line causes memory issues not sure why and it is probably me but it does
			if (GUILayout.Button("Cancel Capture"))
			{
				CancelCapture();
			}
			GUI.backgroundColor = Color.red; // same with thing one
			if (GUILayout.Button("Stop Capture"))
			{
				StopCapture();
			}
		...
		private void StartCapture()
		{
			...
			if (_movieCapture != null)
			{
				Debug.Log("GUI Called Start Capture");
