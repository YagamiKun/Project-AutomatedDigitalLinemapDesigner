...
namespace RenderHeads.Media.AVProMovieCapture
{
	...
	public class CaptureStats
	{
		...
		public float FPSFrameCount { get { return _frameCount; } }
	...
	public partial class CaptureBase : MonoBehaviour
	{
        ...
        [System.Flags]
        public enum FilenameComponents
        {
            None			= 0,
            Date			= 1 << 0,
            Time			= 1 << 1,
		...
		public static readonly FilenameComponents DefaultFilenameComponents
			= FilenameComponents.Date 
			| FilenameComponents.Time
			| FilenameComponents.Resolution 
			| FilenameComponents.VideoCodec 
			| FilenameComponents.AudioCodec 
			| FilenameComponents.FPS;
		...
		//[SerializeField] bool _appendFilenameTimestamp = true; // removed in favor of _filenameComponents
		...
		// add extras to the file name (date, time, version, scene, etc...)
        [SerializeField] FilenameComponents _filenameComponents = DefaultFilenameComponents;
		[SerializeField] string _filenameComponentCustomText = "";
		[SerializeField] string _filenameComponentSeperator = "_";
		private const string _defualtFilenameComponentSeperator = "_";


		public FilenameComponents FileNameComponents
		{
			get { return _filenameComponents; }
			set { _filenameComponents = value; }
		}
...
// 				Debug.LogWarning("[AVProMovieCapture] 'OutputPath.PhotoLibrary' is not available in the Unity Editor, defaulting to 'OutputPath.RelativeToProject'");
		...
		public void AddContentToFileName(ref StringBuilder sb)
		{
			string filenameSeperator = string.IsNullOrEmpty(_filenameComponentSeperator) ? _defualtFilenameComponentSeperator : _filenameComponentSeperator;

			if ((_filenameComponents & FilenameComponents.Date) != 0)
			{
				sb.Append(filenameSeperator).Append(DateTime.Now.ToString("yyyy-MM-dd"));
			}
			if ((_filenameComponents & FilenameComponents.Time) != 0)
			{
				sb.Append(filenameSeperator).Append(DateTime.Now.ToString("HH-mm-ss"));
			}
			if ((_filenameComponents & FilenameComponents.SceneName) != 0)
			{
				sb.Append(filenameSeperator).Append(UnityEngine.SceneManagement.SceneManager.GetActiveScene().name);
			}
			if ((_filenameComponents & FilenameComponents.Platform) != 0)
			{
				sb.Append(filenameSeperator).Append(Application.platform);
			}
			if ((_filenameComponents & FilenameComponents.Resolution) != 0)
			{
				sb.Append(filenameSeperator).Append($"{_targetWidth}x{_targetHeight}");
			}
			if ((_filenameComponents & FilenameComponents.UserName) != 0)
			{
				sb.Append(filenameSeperator).Append(Environment.UserName);
			}
			if ((_filenameComponents & FilenameComponents.CustomText) != 0)
			{
				if (!string.IsNullOrEmpty(_filenameComponentCustomText))
				{
					sb.Append(filenameSeperator).Append(_filenameComponentCustomText);
				}
			}
			if ((_filenameComponents & FilenameComponents.StopMode) != 0)
			{
				switch (_stopMode)
				{
					case StopMode.None:
                        sb.Append(filenameSeperator).Append("None");
                        break;
					case StopMode.FramesEncoded:
                        sb.Append(filenameSeperator).Append($"{_stopFrames}frames");
                        break;
					case StopMode.SecondsEncoded:
						sb.Append(filenameSeperator).Append($"{_stopSeconds}s");
						break;
            ...
            if ((_filenameComponents & FilenameComponents.PixelFormat) != 0)
			{
                sb.Append(filenameSeperator).Append(NativePlugin.GetPixelFormatName(_pixelFormat));
            }
            if ((_filenameComponents & FilenameComponents.VideoCodec) != 0)
			{
                if (_selectedVideoCodec == null)
                {
                    SelectVideoCodec();
                }
                string videoCodec = (_selectedVideoCodec != null) ? _selectedVideoCodec.Name : null;
				if (!string.IsNullOrEmpty(videoCodec))
				{
                    sb.Append(filenameSeperator).Append(videoCodec);
                }
            }
            if ((_filenameComponents & FilenameComponents.AudioCodec) != 0)
            {
                if (_selectedAudioCodec == null)
                {
                    SelectAudioCodec();
                }
                string audioCodec = (_selectedAudioCodec != null) ? _selectedAudioCodec.Name : null;
                if (!string.IsNullOrEmpty(audioCodec))
                {
                    sb.Append(filenameSeperator).Append(audioCodec);
                }
            }
            if ((_filenameComponents & FilenameComponents.FPS) != 0)
			{
                sb.Append(filenameSeperator).Append($"{_frameRate}fps");
            }
        }

        public string GenerateFilePathPreview()
		{
...
#endif
                filePath = GenerateFilePath(outputFolderType, outputFolderPath, fileName);
...
// [MOZ] 250530 - Disabled for now as early cleanup of the recorder breaks the logic further on and prevents any
// completion handlers that may have been added from running
#if false
		...
		public enum PhotoLibraryAuthorisationStatus
		{
			...
			Authorised,
			/// <summary>Limited access to the photo library has been granted.</summary>
			Limited
		};
