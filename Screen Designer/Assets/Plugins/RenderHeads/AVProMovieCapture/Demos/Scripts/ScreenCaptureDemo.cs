...
namespace RenderHeads.Media.AVProMovieCapture.Demos
{
	public class ScreenCaptureDemo : MonoBehaviour
	{
			...
			if (_capture != null)
			{
					...
					_capture.OutputFolder == CaptureBase.OutputPath.RelativeToPictures)
				{
					...
					switch (CaptureBase.HasUserAuthorisationToAccessPhotos(photoLibraryAccessLevel))
					{
						...
						case CaptureBase.PhotoLibraryAuthorisationStatus.Limited:
		...
		private void Update()
		{
			bool playAudioHit = false;

#if (!ENABLE_INPUT_SYSTEM || ENABLE_LEGACY_INPUT_MANAGER)
			// Press the S key or tap the screen to trigger audio and background color change - useful for testing A/V sync
	#if (UNITY_IOS || UNITY_ANDROID) && !UNITY_EDITOR
			playAudioHit = (Input.touchCount > 0 && Input.GetTouch(0).phase == TouchPhase.Ended);
	#else
			playAudioHit = Input.GetKeyDown(KeyCode.S);
	#endif
			...
			if (playAudioHit && _audioHit != null && _capture != null && _capture.IsCapturing())
			{
